...
- **自动跳过已合并**：已存在或补丁内容等价的提交自动跳过，重复运行安全
- **合并前更新目标分支**：自动 `git pull` 将目标分支更新到最新状态
- **冲突可交互处理**：冲突时暂停，支持继续 / 跳过 / 中止 / 保留现场退出
- **简单确认流程**：全程选项式交互（A / B / C / S / E），无需手输长命令
